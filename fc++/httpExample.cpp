...
public:
    explicit
    session(net::any_io_executor ex, ssl::context& ctx) : resolver_(ex), stream_(ex, ctx) {};
    ...
    run(char const* host, char const* port, char const* target, int version)
        ...
        // Set up an HTTP GET request messaget
    ...
    on_read(beast::error_code ec, std::size_t bytes_transferred)
    {
        ...
        std::cout << "THIS IS THE RESPONSE!   " << res_.body() << std::endl;
...
int main(int argc, char** argv)
{
    ...
    //    return EXIT_FAILURE;
    //}
    auto const host = "blanzynetwork.org";
    ...
    ctx.set_verify_mode(ssl::verify_peer); //this most be modified to "verify_peer" on production
